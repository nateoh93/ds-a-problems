...
function stepper(nums) {
    ...
    for (let i=0; i<table.length; i++) {
        if (table[i] === true) {
            let range = nums[i];
            for (let step = i; step <= range; step++) {
                table[i+step] = true;
            }
        }
    }
...
function maxNonAdjacentSum(nums) {
    ...
    for (let i = 1; i<nums.length; i++) {
        let leftNum = table[i-2] === undefined ? 0 : table[i-2];
        let numTotal = leftNum + nums[i];
        let withoutNumTotal = table[i-1];
        table[i] = Math.max(numTotal, withoutNumTotal) 
    }
