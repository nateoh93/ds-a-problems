...
class MinMaxStack {
    ...
    min() {
        return this.minLength[this.minLength.length-1] || null;
    }
    
    max() {
        return this.maxLength[this.maxLength.length-1] || null;
    }

    push(val) {
        const newNode = new Node(val);

        if (!this.minLength.length || newNode.value <= this.minLength[this.minLength.length-1].value) {
            this.minLength.push(newNode);
        }

        if (!this.maxLength.length || newNode.value >= this.maxLength[this.maxLength.length-1].value) {
            this.maxLength.push(newNode);
        }
    ...
    pop() {
        ...
        if (this.top.value === this.minLength[this.minLength.length-1].value) {
            this.minLength.pop();
        }
        
        if (this.top.value === this.maxLength[this.maxLength.length-1].value) {
            this.maxLength.pop();
        }
